...
#_________Serialize and Deserialize________________________
def serialize(root):
  qu = deque([])
  qu.append(root)
  ans = ""
...
def deserialize(s):
  ...
  while len(qu)!=0:
    parent = qu.popleft()
    if lt[pointer]!="N":
      left = Node(lt[pointer])
      qu.append(left)
      parent.left = left
    pointer+=1
    if lt[pointer]!="N":
      right = Node(lt[pointer])
      qu.append(right)
      parent.right = right
    pointer+=1
  return root
...
#---------Flattening th BT to Linked List-------
...
def flatten2(root):
  ...
  while len(st)!=0:
    front = st.pop()
    if front.right:
      st.append(front.right)
    if front.left:
      st.append(front.left)
    if len(st)!=0:
      front.right = st[-1]
    front.left = None
